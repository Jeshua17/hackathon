const actividadesDB = {
  torneovoleibol: {
    title: "TORNEO MIXTO DE VOLEIBOL",
    category: "VOLEIBOL",
    fecha: "DOM 9 AGO",
    lugar: "PARQUE INFANTIL DE FUNDECI",
    cupos: "8 cupos",
    duracion: "4PM",
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const activityId = urlParams.get('id') || 'torneovoleibol';
  
  const data = actividadesDB[activityId] || actividadesDB['torneovoleibol'];
...
window.handleFormSubmit = async function(event) {
  ...
  const actividad = (formTitle?.textContent || detailTitle?.textContent || 'TORNEO MIXTO DE VOLEIBOL').trim();
  ...
  try {
    ...
    if (!respuesta.ok || !datos.exito) {
      alert(datos.mensaje || 'Hubo un problema al procesar la inscripción.');
      return;
    }
    ...
    alert('No se pudo establecer conexión con el servidor. Verifica que Node.js esté activo.');
  }
};
