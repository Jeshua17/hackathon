const actividadesDB = {
  voleibol: {
    title: "VOLEIBOL AMISTOSO",
    category: "VOLEIBOL",
    fecha: "JUE 6 AGO",
    lugar: "CANCHA MODESTO ARMIJO",
    cupos: "12 cupos",
    duracion: "6PM",
    nivel: "Todos los niveles",
    requisitos: "Ropa cómoda y ganas de divertirse",
    img: "https://images.unsplash.com/photo-1666901356149-93f2eb3ba5a2?w=1000&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8M3x8dm9sZWlib2x8ZW58MHx8MHx8fDA%3D"
  }
};

document.addEventListener('DOMContentLoaded', () => {
  const urlParams = new URLSearchParams(window.location.search);
  const activityId = urlParams.get('id') || 'voleibol';
  
  const data = actividadesDB[activityId] || actividadesDB['voleibol'];
...
window.handleFormSubmit = async function(event) {
  ...
  const actividad = (formTitle?.textContent || detailTitle?.textContent || 'VOLEIBOL AMISTOSO').trim();
  ...
  try {
    ...
    if (!respuesta.ok || !datos.exito) {
      alert(datos.mensaje || 'Hubo un problema al registrar la inscripción.');
      return;
    }
