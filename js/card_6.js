...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const activityId = urlParams.get('id') || 'convivencia';
  
  const data = actividadesDB[activityId] || actividadesDB['convivencia'];
...
window.handleFormSubmit = async function(event) {
  ...
  const actividad = (formTitle?.textContent || detailTitle?.textContent || 'CONVIVENCIA JUVENIL').trim();

  try {
    // Se actualizó el endpoint a /api/participantes/inscribir
    ...
    if (!respuesta.ok || !datos.exito) {
      alert(datos.mensaje || 'Ocurrió un error al guardar la inscripción.');
      return;
    }
    ...
    alert('No se pudo establecer comunicación con el servidor. Verifica que Node.js esté en ejecución.');
  }
};
