const actividadesDB = {
  skateboard: {
    title: "SKATEBOARD URBANO",
    category: "URBANO",
    fecha: "VIE 1 AGO",
    lugar: "SKATEPARK - RIO CHIQUITO",
    cupos: "5 cupos",
    duracion: "6PM",
    nivel: "Todos los niveles",
    requisitos: "Ropa cómoda y su skateboard",
    img: "https://images.unsplash.com/photo-1520045892732-304bc3ac5d8e?q=80&w=800&auto=format&fit=crop"
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const activityId = urlParams.get('id') || 'skateboard';
  
  const data = actividadesDB[activityId] || actividadesDB['skateboard'];
...
window.handleFormSubmit = async function(event) {
  ...
  const actividad = (formTitle?.textContent || detailTitle?.textContent || 'SKATEBOARD URBANO').trim();
